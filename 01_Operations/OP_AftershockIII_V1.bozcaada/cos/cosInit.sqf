...
COSmaxGrps=25;//Set Maximum group limit for COS at any one time (If limit is hit then civilians will be placed into a single group for each town)
...
_randomisation=2;
	...
	_parked=3;// Parked Vehicles Spawned
 	...
 	if (_houses > 140) 
		then {
	_civilians=18+ round(random _randomisation);// Civilians spawned
	_vehicles=5;// Moving Vehicles Spawned
	_parked=3;// Parked Vehicles Spawned
			};
