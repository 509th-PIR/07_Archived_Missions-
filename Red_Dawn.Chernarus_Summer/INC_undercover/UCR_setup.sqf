...
_regDetectRadius = 1;                  //Default detection radius for regular troops (this will expand and contract based on weather, time of day, and how the undercover unit is acting - civilians within this radius will be under much more scrutinty)

_asymEnySide = west;             //Units of this side will be classed as asymetric enemies (Side: can be east, west, independent) - if you don't need this, comment the line out (i.e. put // before _asymEnySide, as in //_asymEnySide = east;).
...
_civFactions = ["CIV_F","CIV_F_TANOA","LOP_CHR_Civ"]; //Array of factions whose vests are safe for undercover units to wear
...
_incogFactions = ["OPF_F","OPF_T_F","rhs_faction_vdv","rhs_faction_msv","rhs_faction_tv"]; //Array of enemy factions whose items and vehicles will allow the player to impersonate the enemy
