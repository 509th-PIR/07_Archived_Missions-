...
if (!(getmarkercolor _mkr == "colorblack")) then {
	...
	for "_counter" from 1 to _cGrps do {
		...
		if ((_cSize select 0) > 0) then {0=[(_cGroup select 0),_cSize,(_cGroup select 2),_faction,_cargoType] call eos_fnc_setcargo_civ;};
	...
	if ((_fSize select 0) > 0) then {
		...
		0=[(_fGroup select 2),"civAIRskill"] call eos_fnc_civ_grouphandlers;
