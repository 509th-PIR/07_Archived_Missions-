...
RYD_HAS_atCalled = 
	...
	if (((_pickPos distance RYD_HAS_Chopper) < 100) and {(RYD_HAS_whenInside) and {(RYD_HAS_radioCalled) and {not ((RYD_HAS_isSupplyRun) or {(RYD_HAS_isCASRun)})}}}) exitWith
		{
...
RYD_HAS_GiveActions = 
	...
	if not (RYD_HAS_UseSupportsMenu) then
		...
		private _cond3 = "((_originalTarget getVariable ['RYD_HAS_canCall',true]) and {(((RYD_HAS_wasCalled) and {(isPlayer _originalTarget) and {(_this == _originalTarget) and {not (isNil 'RYD_HAS_Chopper') and {not (isNull RYD_HAS_Chopper) and {(alive RYD_HAS_Chopper)}}}}}) and {(not (RYD_HAS_isSupplyRun) or {not (RYD_HAS_SupplyDrop_SlingLoad) or {(RYD_HAS_Hooked) or {(RYD_HAS_delivered)}}})})})";
			...
			[_x,["<t color='#fe8701'>Call for transport</t>", "[_this,RYD_HAS_atCalled] remoteExec ['spawn',2];", 0, 1, false, true, "", _cond],"RYD_HAS_myCallAction"] remoteExec ["RYD_HAS_AddAction",_x];
			...
			[_x,["<t color='#fe8701'>Call for supplies</t>", "[_this,RYD_HAS_atCalled] remoteExec ['spawn',2];", 1, 1, false, true, "", _cond],"RYD_HAS_myCallAction2"] remoteExec ["RYD_HAS_AddAction",_x];
			...
			[_x,["<t color='#fe8701'>Call for CAS</t>", "[_this,RYD_HAS_atCalled] remoteExec ['spawn',2];", 2, 1, false, true, "", _cond],"RYD_HAS_myCallAction3"] remoteExec ["RYD_HAS_AddAction",_x];
			...
			while {true} do
				...
				if ((RYD_HAS_RemoveAtLimit) and {(not ((RYD_HAS_Limit - RYD_HAS_UseCount) > 0) and {not (RYD_HAS_Limit < 0)})}) exitWith 
					...
					if (not (isNull _x) and {(alive _x)}) then
						...
						_cond3 = ((_x getVariable ['RYD_HAS_canCall',true]) and {(((RYD_HAS_wasCalled) and {not (isNil 'RYD_HAS_Chopper') and {not (isNull RYD_HAS_Chopper) and {(alive RYD_HAS_Chopper)}}}) and {(not (RYD_HAS_isSupplyRun) or {not (RYD_HAS_SupplyDrop_SlingLoad) or {(RYD_HAS_Hooked) or {(RYD_HAS_delivered)}}})})});	
